...
**3.3.2.3.** $β_{H,i}$ is the heating coefficient for site $i$. It represents the incremental change in energy use per day for every additional heating degree day.
{: data-depth="4"}

**3.3.2.4.** $β_{C,i}$ is the cooling coefficient for site $i$. It represents the incremental change in energy use per day for every additional cooling degree day.
{: data-depth="4"}
...
**3.3.3.3.** $Σ(U_d)$ is the sum of all daily use values $U_d$ for a given period $p$.
{: data-depth="4"}
...
**3.8.3.2.** $\mu_i$ is the mean use for the site.
{: data-depth="4"}

**3.8.3.3.** $\beta_{H_i}$ is the heating coefficient.
{: data-depth="4"}

**3.8.3.4.** $HDH50_p$ is the heating degree-hour with a 50-degree balance point.
{: data-depth="4"}

**3.8.3.5.** $\beta_{C_i}$ is the cooling coefficient.
{: data-depth="4"}

**3.8.3.6.** $CDH65_p$ is the cooling degree hour with a 65-degree balance point.
{: data-depth="4"}
...
**3.9.1.3.** If the temperature $T_p > B_1$, then the first temperature feature $Tc_{1, p} = B_1$ and the algorithm proceeds to the next step. Otherwise, $Tc_{1, p} = T_p$, and $Tc_{n, p} = 0$, for $n = 2...N$, and the algorithm ends.
{: data-depth="4"}

**3.9.1.4.** For $n = 2...N$, if the temperature $T_p > B_n$, then $Tc_{n, p} = B_n - B_{n-1}$ and the algorithm proceeds to the next $n$. Otherwise, $Tc_{n, p} = T_p - B_{n-1}$, and $Tc_{n, p} = 0$, for $n = (n+1)...N$, and the algorithm ends.
{: data-depth="4"}

**3.9.1.5.** If the temperature $T_p > B_N$, then the last temperature feature $Tc_{N+1, p} = T_p - B_N$, and equal to zero otherwise.
{: data-depth="4"}
